...
public class ContactPage extends BasePage {
    ...
    @FindBy(id = "forename") private WebElement forenameInput;           // Input field for Forename
    @FindBy(id = "email") private WebElement emailInput;                 // Input field for Email
    @FindBy(id = "message") private WebElement messageInput;             // Input field for Message
    @FindBy(xpath = "//a[text()='Submit']") private WebElement submitBtn; // Submit button element
    @FindBy(css = ".alert.alert-success") private WebElement successMsg;  // Success alert message element
    @FindBy(css = ".alert-error li") private WebElement errorMessages;    // List item elements for individual error messages
    @FindBy(css = ".alert-error") private WebElement headerErrorMessage;  // Header error message element
    @FindBy(css = "span.help-inline") private List<WebElement> fieldErrorMessages; // Inline error messages per field
    ...
    public List<String> getFieldErrorMessages() {
        return fieldErrorMessages.stream()
                                 .map(WebElement::getText)
                                 .toList();
    }

    // Returns the list of WebElements representing inline error messages (for validation in tests)
